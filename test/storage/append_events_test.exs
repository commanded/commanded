defmodule EventStore.Storage.AppendEventsTest do
  ...
  test "append single event to existing stream", %{storage: storage} do
    ...
    {:ok, 1} = Storage.append_to_stream(storage, stream_uuid, 0, EventFactory.create_events(1))
    {:ok, 1} = Storage.append_to_stream(storage, stream_uuid, 1, EventFactory.create_events(1))
  end

  test "append multiple events to existing stream", %{storage: storage} do
    stream_uuid = UUID.uuid4()

    {:ok, 3} = Storage.append_to_stream(storage, stream_uuid, 0, EventFactory.create_events(3))
    {:ok, 3} = Storage.append_to_stream(storage, stream_uuid, 3, EventFactory.create_events(3))
  end
  ...
  test "append to new stream, but stream already exists", %{storage: storage} do
    ...
    {:ok, 1} = Storage.append_to_stream(storage, stream_uuid, 0, events)
