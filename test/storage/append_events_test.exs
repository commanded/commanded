defmodule EventStore.Storage.AppendEventsTest do
  ...
  test "append single event to existing stream", %{store: store} do
    ...
    {:ok, 1} = Storage.append_to_stream(store, uuid, 0, EventFactory.create_events(1))
    {:ok, 1} = Storage.append_to_stream(store, uuid, 1, EventFactory.create_events(1))
  end

  test "append multiple events to existing stream", %{store: store} do
    uuid = UUID.uuid4()

    {:ok, 3} = Storage.append_to_stream(store, uuid, 0, EventFactory.create_events(3))
    {:ok, 3} = Storage.append_to_stream(store, uuid, 3, EventFactory.create_events(3))
  end

  test "append events to different, new streams", %{store: store} do
    {:ok, 2} = Storage.append_to_stream(store, UUID.uuid4(), 0, EventFactory.create_events(2))
    {:ok, 2} = Storage.append_to_stream(store, UUID.uuid4(), 0, EventFactory.create_events(2))
  end
  ...
  test "append events to different, existing streams", %{store: store} do
    ...
    {:ok, 2} = Storage.append_to_stream(store, stream1_uuid, 0, EventFactory.create_events(2))
    {:ok, 2} = Storage.append_to_stream(store, stream2_uuid, 0, EventFactory.create_events(2))
    {:ok, 2} = Storage.append_to_stream(store, stream1_uuid, 2, EventFactory.create_events(2))
    {:ok, 2} = Storage.append_to_stream(store, stream2_uuid, 2, EventFactory.create_events(2))
  end
